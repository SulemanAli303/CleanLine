...
import UIKit
import Cosmos
class ChaletsBooking: BaseViewController {
    ...
    var SellerData : SellerData? {
        didSet {
            ...
            bannerImg.sd_setImage(with: URL(string: SellerData?.banner_image ?? ""), placeholderImage: #imageLiteral(resourceName: "placeholder_banner"))
    ...
    @IBAction func menuPressed(_ sender: UIButton) {
        Coordinator.goToCharletsListVC(controller: self, venderID: chaletID)
    }
    func setProductCell() {
...
extension ChaletsBooking: UICollectionViewDataSource,UICollectionViewDelegate ,UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.productImage.image = UIImage(named: "Rectangle 2227")
        return cell
    }
...
extension ChaletsBooking {
    func getStoreDetailsAPI() {
        let parameters:[String:String] = [
            "access_token" : SessionManager.getAccessToken() ?? "",
            "seller_user_id" : chaletID
        ]
