//
//  TabelBookingViewController.swift
//  LiveMArket
//
...
class TabelBookingViewController: BaseViewController {
    ...
    var myOrdersList:[MyTableBookingList] = []
    var receivedOrdersList:[MyTableBookingList?] = []
    
    var pageCount = 1
    var pageLimit = 10
    var hasFetchedAll = false
    var userData: User?
    var isReceivedEnable:Bool = false
    var isPopRoot = false
    var myOrdersData:MyTableBookingListData?{
        didSet{
            ...
            self.myOrdersList.append(contentsOf: myOrdersData?.list ?? [MyTableBookingList]())
            DispatchQueue.main.async {
                self.tableView.reloadData()
            }
        }
    }
    var myReceivedOrdersData:MyTableBookingListData?{
        didSet{
            ...
            if isFromNewOrdersFromProfile{
                ...
                for index in 0..<(myReceivedOrdersData?.list?.count ?? 0) {
                    if myReceivedOrdersData?.list?[index].booking_status == "0"{
                ...
                self.receivedOrdersList.removeAll()
                for index in 0..<(myReceivedOrdersData?.list?.count ?? 0) {
                    if myReceivedOrdersData?.list?[index].booking_status == "1" || myReceivedOrdersData?.list?[index].booking_status == "2" || myReceivedOrdersData?.list?[index].booking_status == "3" || myReceivedOrdersData?.list?[index].booking_status == "4" ||
                        myReceivedOrdersData?.list?[index].booking_status == "5"{
                        self.receivedOrdersList.append(myReceivedOrdersData?.list?[index])
                    }
                }
            }else{
                self.receivedOrdersList.append(contentsOf: myReceivedOrdersData?.list ?? [MyTableBookingList]())
            }
            DispatchQueue.main.async {
                self.tableView.reloadData()
            }
...
//        appDelegate.stopAlaram()
    ...
    @objc func setupNotificationObserver()
    {
        ...
        if isFromDriver{
            ...
            if view.safeAreaInsets.bottom >= 0 {
                ...
                topViewHeightConstraint.constant = 140
            } else {
                topView.isHidden = false
                topViewHeightConstraint.constant = 140
            }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        tabbar?.hideTabBar()
        viewControllerTitle = "My Table Bookings"
...
//            }
    ...
    func myOrdesListAPI() {
        ...
        print(parameters)
        StoreAPIManager.myTableBookingListAPI(parameters: parameters) { result in
    ...
    func myReceivedOrdersAPI() {
        ...
        StoreAPIManager.myRecevedTableBookingListAPI(parameters: parameters) { result in
            switch result.status {
            case "1":
...
extension TabelBookingViewController : UITableViewDelegate ,UITableViewDataSource{
...
//        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if isMyOrderListClicked{
            ...
            Cell.dateLabel.text = ""
            if myOrdersList.count > indexPath.row {
                Cell.myTableBookingList = self.myOrdersList[indexPath.row]
            }
            ...
            Cell.selectionStyle = .none
            Cell.dateLabel.text = ""
            if receivedOrdersList.count > indexPath.row {
                Cell.myTableBookingList = receivedOrdersList[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if isMyOrderListClicked{
            Coordinator.goToTableRequest(controller: self,bookID: self.myOrdersList[indexPath.row].id ?? "",isFromReceved: false,isFromPopup:  false)
        }else{
            Coordinator.goToTableRequest(controller: self,bookID: self.receivedOrdersList[indexPath.row]?.id ?? "",isFromReceved: true,isFromPopup: false)
        }
    }
...
extension TabelBookingViewController:MyBookingDelegate{
  
    func gotoDetailsForOrderId(orderId: String) {
        guard let index = self.receivedOrdersList.firstIndex(where: { $0?.id == orderId }) else {
            return
        }
        Coordinator.goToTableRequest(controller: self,bookID: self.receivedOrdersList[index]?.id ?? "",isFromReceved: true,isFromPopup: false)
    }
