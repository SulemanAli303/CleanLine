//
//  PGBookingListViewController.swift
//  LiveMArket
//
...
class PGBookingListViewController: BaseViewController {
    ...
    var receivedOrdersList:[GroundReceivedBookingList?] = []
    ...
    var myBookingData:PGBookingData?{
        didSet{
            ...
            self.myBookingList.append(contentsOf: myBookingData?.list ?? [PGBookingList]())
    ...
    var myReceivedBookingData:GroundReceivedBookingData?{
        didSet{
            ...
            if isFromNewOrdersFromProfile{
                ...
                self.receivedOrdersList.append(contentsOf: myReceivedBookingData?.list ?? [GroundReceivedBookingList]())
            }
...
//        appDelegate.stopAlaram()
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if userData?.user_type_id == "2"{
            if userData?.activity_type_id == "17"{
...
//        tabbar?.showTabBar()
    ...
    func myOrdesListAPI() {
        ...
        print(parameters)
        StoreAPIManager.myPGBookingListAPI(parameters: parameters) { result in
...
//            "limit":pageLimit.description
        ...
        print(parameters)
        StoreAPIManager.myPGSellerBookingListAPI(parameters: parameters) { result in
            switch result.status {
            case "0":
...
extension PGBookingListViewController : UITableViewDelegate ,UITableViewDataSource{
...
//        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if isMyOrderListClicked{
            ...
            Cell.myBookingList = self.myBookingList[indexPath.row]
            ...
            Cell.selectionStyle = .none
            Cell.myPGBookingList = receivedOrdersList[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if self.isMyOrderListClicked{
            PlayGroundCoordinator.goToBookingDetail(controller: self,isFromReceive: false,booking_ID: myBookingList[indexPath.row].booking_id ?? "")
        }else{
            PlayGroundCoordinator.goToBookingDetail(controller: self,isFromReceive: true,booking_ID: receivedOrdersList[indexPath.row]?.booking_id ?? "")
        }
...
//                if isMyOrderListClicked{
//                    SellerOrderCoordinator.goToFoodOrderDetail(controller: self,orderId: self.myBookingList[indexPath.row].invoice_id ?? "",isSeller: false,isThankYouPage: false)
//                }else{
//                    SellerOrderCoordinator.goToFoodOrderDetail(controller: self,orderId: self.receivedOrdersList[indexPath.row].booking_id ?? "",isSeller: true,isThankYouPage: false)
//                }
                
 ///           }else{
//                if isMyOrderListClicked{
//                    SellerOrderCoordinator.goToSellerBookingDetail(controller: self,orderId: self.myBookingList[indexPath.row].booking_id ?? "",isSeller: false, isThankYouPage: false)
//                }else{
...
//                }else
//                {
//                    SellerOrderCoordinator.goToSellerBookingDetail(controller: self,orderId: self.myBookingList[indexPath.row].order_id ?? "",isSeller: false, isThankYouPage: false)
//                }
//            }else{
//                SellerOrderCoordinator.goToSellerBookingDetail(controller: self,orderId: self.receivedOrdersList[indexPath.row].order_id ?? "",isSeller: true, isThankYouPage: false)
//            }
...
//extension PGBookingListViewController:GroundBookingDelegate{
//
//    func refreshList() {
//        resetPage()
//        self.myReceivedOrdersAPI()
...
extension PGBookingListViewController:MyPGBookingPorocol{
...
extension PGBookingListViewController:MyBookingDelegate{
    func gotoDetailsForOrderId(orderId: String) {
        ...
        PlayGroundCoordinator.goToBookingDetail(controller: self,isFromReceive: true,booking_ID: receivedOrdersList[index]?.booking_id ?? "")
    }
