...
import Foundation
struct MyTableBookingList_Base : Codable {
    let status : String?
    let message : String?
    let oData : MyTableBookingListData?
    ...
    init(from decoder: Decoder) throws {
        ...
        oData = try values.decodeIfPresent(MyTableBookingListData.self, forKey: .oData)
    }

}
struct MyTableBookingListData : Codable {
    let list : [MyTableBookingList]?
    ...
    init(from decoder: Decoder) throws {
        let values = try decoder.container(keyedBy: CodingKeys.self)
        list = try values.decodeIfPresent([MyTableBookingList].self, forKey: .list)
    }

}
struct MyTableBookingList : Codable {
    let id : String?
