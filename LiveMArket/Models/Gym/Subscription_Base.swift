...
import Foundation
struct Subscription_Base : Codable {
    let status : String?
    let message : String?
    let oData : SubscriptionData?
    ...
    init(from decoder: Decoder) throws {
        ...
        oData = try values.decodeIfPresent(SubscriptionData.self, forKey: .oData)
    }
...
struct SubscriptionData : Codable {
    ...
    init(from decoder: Decoder) throws {
        let values = try decoder.container(keyedBy: CodingKeys.self)
        list = try values.decodeIfPresent([SubscriptionList].self, forKey: .list)
...
struct SubscriptionList : Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        user = try values.decodeIfPresent(SubscriptionUser.self, forKey: .user)
    }
