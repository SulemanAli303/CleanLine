...
@IBDesignable public class TrimmerView: AVAssetTimeSelector {
    ...
    @IBInspectable public var mainColor: UIColor = UIColor(hex: "EF9417") {
        didSet {
            updateMainColor()
        }
    }
    ...
    private var currentPositionBarConstraint: CGFloat = 0
    ...
    private func setupGestures() {
        ...
        if #available(iOS 13.4, *) {
            positionBarPanGestureRecognizer.allowedScrollTypesMask = [.all]
        } else {
            // Fallback on earlier versions
        }
        trimView.addGestureRecognizer(positionBarPanGestureRecognizer)
    }
    ...
    @objc func handlePanGesture(_ gestureRecognizer: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            if isLeftGesture {
                ...
                updatePostionBarConstraint(with: translation)
            }
            layoutIfNeeded()
    ...
    private func updatePostionBarConstraint(with translation: CGPoint) {
//        let maxConstraint = min(2 * handleWidth - frame.width + leftHandleView.frame.origin.x + minimumDistanceBetweenHandle, 0)
//        let newConstraint = max(min(0, currentRightConstraint + translation.x), maxConstraint)
        positionConstraint?.constant = translation.x
    }
