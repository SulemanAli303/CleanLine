...
public extension PIPUsable {
    ...
    var initialPosition: PIPPosition { return .middleRight }
    ...
    var pipSize: CGSize { return CGSize(width: 120.0, height: 100) }
    var pipShadow: PIPShadow? { return PIPShadow(color: .black, opacity: 0.5, offset: CGSize(width: 0, height: 8), radius: 10) }
    var pipCorner: PIPCorner? {
        if #available(iOS 13.0, *) {
            return PIPCorner(radius: 10, curve: .continuous)
        } else {
            return PIPCorner(radius: 10, curve: nil)
        }
    }
    func didChangedState(_ state: PIPState) {}
    func didChangePosition(_ position: PIPPosition) {}
